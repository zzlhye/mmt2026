						...
						<tr>
							...
							<td class="col-title">
								<a href="javascript:void(0);"
									onclick="selectMenti('${user.userId}', '${user.name}'); return false;">
									${user.name}
								</a>
							</td>
			...
			<nav class="mento-pager" aria-label="페이지네이션">
				...
				<c:choose>
					...
					<c:when test="${pageMaker.prev}">
						<button class="pager-chev"
							type="button"
							onclick="location.href='/team/mentiList${pageMaker.makeSearch(pageMaker.startPage - 1)}'">
							&lt;
						</button>
					</c:when>
				...
				<c:forEach begin="${pageMaker.startPage}"
					...
					<button
						...
						type="button"
						onclick="location.href='/team/mentiList${pageMaker.makeSearch(idx)}'">
				...
				<c:choose>
					...
					<c:when test="${pageMaker.next}">
						<button class="pager-chev"
							type="button"
							onclick="location.href='/team/mentiList${pageMaker.makeSearch(pageMaker.endPage + 1)}'">
							&gt;
						</button>
					</c:when>
...
<script>
	...
	function goSearch() {
		...
		const url = new URL(
			window.location.origin + "/team/mentiList"
		);
	...
	// 멘티 선택
	function selectMenti(userId, name) {

		if (opener && !opener.closed) {
			opener.setMentiId(userId, name);
		}

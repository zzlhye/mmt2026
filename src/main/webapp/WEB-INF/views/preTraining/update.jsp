...
<div class="notice-update__container">
	...
	<div class="notice-update__title">사전교육 수정</div>
	<div class="notice-update__divider"></div>

	<form action="/preTraining/update" name="frm"
		class="notice-update__form" method="post">
		...
		<div class="notice-update__row">
			...
			<div class="notice-update__field">
				<input id="title" name="title" class="notice-update__input"
					value="${preTraining.title}" type="text">
			</div>
		...
		<div class="notice-update__row">
			...
			<label class="notice-update__label" for="category"> 대상 </label>

			<div class="notice-update__field">

				<div class="notice-update__select-wrap">

					<select id="category" name="category" class="notice-update__select">

						<option value="">선택</option>

						<option value="A"
							<c:if test="${preTraining.category eq 'A'}">selected</c:if>>
							전체</option>

						<option value="M"
							<c:if test="${preTraining.category eq 'M'}">selected</c:if>>
							멘토</option>

						<option value="S"
							<c:if test="${preTraining.category eq 'S'}">selected</c:if>>
							멘티</option>

					</select> <span class="notice-update__caret">▼</span>
		...
		<div class="notice-update__row">
			...
			<div class="notice-update__field">
				<textarea id="content" name="content"
					class="notice-update__textarea">${preTraining.content}</textarea>
			</div>
		...
		<div class="notice-update__row">
			...
			<div class="notice-update__field">
				...
				<div class="notice-update__file-row">
					...
					<label for="fileUpload" class="notice-update__file-btn">
						파일선택 </label> <input type="hidden" id="uploadCount"
						value="${fn:length(files)}"> <input id="fileUpload"
						type="file" class="notice-update__file-input" multiple>
						...
						<div class="row align-items-center existing-file-row">
							...
							<div class="col-auto">
								...
								<a href="javascript:void(0);"
									...
									style="padding-left: 5px;"> <c:out value="${file.fileName}" />
								</a>
		...
		<div class="notice-update__actions">
			...
			<button type="button"
				...
				onclick="updatePreTraining()">수정</button>

			<button type="button"
				class="notice-update__btn notice-update__btn--ghost"
				onclick="location.href='/preTraining/read?preNum=${preTraining.preNum}'">
				취소</button>
...
<script>
	...
	function updatePreTraining() {
		...
		if (confirm("해당 사전교육을 수정하시겠습니까?")) {

			$(".uploadedList .delbtn").each(
					function() {

						$("<input>").attr("type", "hidden").attr("name",
								"files").val($(this).attr("href")).appendTo(
								document.frm);
	...
	$(".btn_onlyFileDelete").on(
			...
			function() {
				...
				$.ajax({
					url : "/preTraining/onlyFileDelete",
					type : "POST",
					data : {
						fileNum : fileNum
					},
					...
					success : function(result) {
						...
						if (result == "deleted") {
							...
							var uploaded = parseInt($("#uploadCount").val()
									|| "0", 10);

							$("#uploadCount").val(Math.max(uploaded - 1, 0));

...
|  | Spring Framework | 애플리케이션 구성 및 계층별 컴포넌트 관리 |
|  | Spring MVC | HTTP 요청·응답 처리 및 MVC 구조 구현 |
|  | MyBatis | Mapper 기반 데이터 접근 및 SQL 처리 |
| **Frontend** | JSP / JSTL | 서버 사이드 View 및 동적 화면 구성 |
|  | HTML5 / CSS3 | 화면 구조 및 반응형 UI 구현 |
|  | JavaScript / jQuery | 클라이언트 동작, DOM 및 이벤트 처리 |
|  | Ajax | 파일 업로드·댓글·반응 등 비동기 처리 |
| **Database** | Oracle | 관계형 데이터 저장 및 관리 |
| **개발 환경** | STS | Spring 프로젝트 개발 |
|  | Apache Tomcat | 웹 애플리케이션 실행 |
|  | SQL Developer | Oracle DB 관리 |
...
멘토링 프로그램의 **사용자 → 팀 → 주차 → 멘토링 활동**을 중심으로 데이터를 관리할 수 있도록 설계했습니다.
...
| `TBL_USER` | 사용자 정보 |
| `TBL_TEAM` | 멘토링 팀 |
| `TBL_TEAM_MEMBER` | 팀 구성원 |
| `TBL_WEEKLY` | 주차별 활동 |
| `TBL_MENTORING` | 멘토링 게시글 |
| `TBL_MENTORING_FILE` | 게시글 첨부파일 |
| `TBL_MENTORING_COMT` | 게시글 댓글 |
| `TBL_MENTORING_RELATE` | 게시글 반응 |
...
| 구분 | 기능 |
| --- | --- |
| **멘토·멘티 관리** | 멘토·멘티 등록·조회·수정·삭제, 사용자 유형 구분 |
| **팀 관리** | 멘토링 팀 등록·조회·수정·삭제, 팀 소개 등록·수정 |
| **주차 관리** | 주차 등록·조회·수정·삭제, 주차별 활동 및 활동 기간 설정 |
| **멘토링 관리** | 주차별 활동 기록 CRUD, 다중 첨부파일, 댓글 및 게시글 반응 |
| **공지사항 관리** | 게시글 CRUD, 게시 대상(전체·멘토·멘티) 설정, 다중 첨부파일 |
| **사전교육 관리** | 교육자료 CRUD, 게시 대상(전체·멘토·멘티) 설정, 다중 첨부파일 |
| **패트릿 보드 관리** | 멘토링 후기 등록·목록 조회·수정·삭제 |
...
전체 기능 중 **멘토링 운영의 핵심 흐름과 주요 구현 기능**을 중심으로 정리했습니다.
...
> 주차별 활동명과 활동 기간을 관리하고 현재 진행 중인 주차를 바로 확인할 수 있습니다.

<img src="images/weekly1.png" width="850" alt="주차별 활동 관리1"> 
<img src="images/weekly2.png" width="850" alt="주차별 활동 관리2"> 
...
> 위에서 소개한 주요 기능을 포함하여 공지사항, 사전교육, 패트릿 보드 등 프로젝트의 전체 사용 흐름을 확인할 수 있습니다.
...
멘토링 활동을 팀과 주차를 기준으로 관리할 수 있도록 데이터를 구성했습니다.
...
이를 통해 특정 팀의 주차별 활동과 해당 주차에 작성된 멘토링 기록을 연결할 수 있도록 했습니다.
...
| `service` | 비즈니스 로직 처리 |
| `persistence` | DAO를 통한 데이터 접근 |
| `domain` | VO 및 페이징 관련 객체 |
| `dto` | 계층 간 데이터 전달 객체 |
| `Interceptor` | 로그인 처리 전후의 세션 관리 |
| `util` | 파일 업로드 관련 공통 기능 |
